...
def main():
    ...
    for line in sys.stdin:
        if line.startswith('#'): 
            ...
            chrom, pos = (tokens[0], int(tokens[1]))
            if ((chrom == pchroms[0]) and (pbegs[0] <= pos) and (pos < pends[0]) and (sample in del19accs)):
                del19candidates.append(qual, line.strip())
            elif ((chrom == pchroms[1]) and (pbegs[1] <= pos) and (pos < pends[1]) and (sample in ins20accs)):
                isn20candidates.append(qual, line.strip())
            else:
                pass # print(line.strip())
    if sample in del19accs: 
        assert (sample in del19accs), ('Error, some variant calls (each one can be either false positive or false negative) ' + 
